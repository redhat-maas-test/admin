    ...
    stage('system tests') {
        withCredentials([usernamePassword(credentialsId: '8957fba6-7473-40f6-8593-efefa9e42251', passwordVariable: 'OPENSHIFT_PASSWD', usernameVariable: 'OPENSHIFT_USER')]) {
            withEnv(['SCRIPTS=https://raw.githubusercontent.com/EnMasseProject/travis-scripts/master']) {
                ...
                sh 'export OPENSHIFT_PROJECT=`echo $JOB_NAME | tr / -`; curl -s
                ${SCRIPTS}/run-tests.sh | bash /dev/stdin "" enmasse/install jboss-amqmaas-1-tech-preview/amqmaas10-addresscontroller-openshift jboss-amqmaas-1-tech-preview/amqmaas10-configserv-openshift jboss-amqmaas-1-tech-preview/amqmaas10-queuescheduler-openshift'
            }
        }
    }
